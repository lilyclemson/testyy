...
dClosest0 := TOPN(GROUP(dDistances0,x, ALL), 1, value);
//closest := TOPN(dClosest, 1, value)
//secodClosest := TOPN(dClosest, 1, -value)
//dClosest0 := TOPN(GROUP(TOPN(GROUP(dDistance0,x,ALL,LOCAL),2,value),x,ALL), 2, value) if the dataset is larger than 1m
...
//dTrack1 := TOPN(GROUP(dDistances0,x, ALL), 2, value);// group(....,ALL) will redistribute the value if needed.
//
//d1 := dTrack1[2];

//OUTPUT(dTrack0, NAMED('dTrack0'));
//OUTPUT(dTrack1, NAMED('dTrack1'));
//OUTPUT(d1, NAMED('d1'));
...
lbs0_iniTemp1 := GROUP(JOIN(lbs0_iniTemp, Gt, LEFT.y = RIGHT.x, TRANSFORM(Mat.Types.Element,SELF.y := RIGHT.y; SELF := LEFT;), LOOKUP),x, y, LOCAL);
lbs0_ini := TOPN(lbs0_iniTemp1, 1, value);
...
//			lb0 := TOPN(GROUP(lbs0, x, ALL, LOCAL), 1, value);
...
//			OUTPUT(deltaG1,NAMED('deltaG1')); 
			...
			deltacg :=JOIN(Gt, deltac,  RIGHT.id = LEFT.x, TRANSFORM(Mat.Types.Element,SELF.value := RIGHT.value; SELF := LEFT;), LOOKUP);
			OUTPUT(deltacg, NAMED('delatcg'));
			deltacGt := TABLE(deltacg, {y, v:=MAX(GROUP,value)}, y, FEW, UNSORTED); 
			output(deltacGt,NAMED('deltacGt'));
			deltaG1 := TABLE(deltacGt, {y, value := v});
			OUTPUT(deltaG1,NAMED('deltaG1')); 
...
//			dMappedDistances1 := SORT(MappedDistances(d01,dCentroid1,fDist,dMap1), x, value);	
			ub1_changed_temp := TOPN(GROUP(MappedDistances(d01,dCentroid1,fDist,dMap1),x, ALL), 1, value);
...
//			ub1_changed_final := ML.Cluster.Closest(dMappedDistances2);
			ub1_changed_final := TOPN(GROUP(MappedDistances(d01,dCentroid1,fDist,dMap2),x, ALL), 1, value);
...
c2:=2;			
			...
			//calculate the deltaC
			deltac1 := dDistanceDelta(c2,c2-1,dAddedx1);
			...
			//get deltaG
			deltacg2 :=JOIN(deltac1, Gt, LEFT.id = RIGHT.x, TRANSFORM(Mat.Types.Element,SELF.value := LEFT.value; SELF := RIGHT;));
			OUTPUT(deltacg2, NAMED('delatcg2'));
			deltacGt2 := SORT(deltacg2, y,-value);
			output(deltacGt2,NAMED('deltacGt2'));
			deltaG2 := DEDUP(deltacGt2,y);
			...
			lbs2_temp := JOIN(lbs1, deltaG2, LEFT.y = RIGHT.y , TRANSFORM(Mat.Types.Element, SELF.value := ABS(LEFT.value - RIGHT.value); SELF := LEFT));
			OUTPUT(lbs2_temp, NAMED('lbs2_temp'));
		
			// groupfilter1
			//calculate the 'b(x) for all x
			dMap21 := PROJECT(ub1, TRANSFORM(ClusterPair, SELF.id := LEFT.x; SELF.clusterid := LEFT.y; SELF.number := 0; SELF.value01 := LEFT.value; SELF.value02 := 0; SELF.value03 := 0;));		
			dMappedDistances21 := SORT(MappedDistances(d01,dCentroid2,fDist,dMap21), x, value);	
			OUTPUT(dMappedDistances21, NAMED('dMappedDistances21'));
			ub2_changed_temp := ML.Cluster.Closest(dMappedDistances21);
      ...
      OUTPUT(groupFilter2, NAMED('groupFilter2'));			
			...
			dMap22_temp := JOIN(groupFilter2, Gt, LEFT.y = RIGHT.y, TRANSFORM(Mat.Types.Element, SELF.x := LEFT.x, SELF.y := RIGHT.x, SELF.value := LEFT.value ));
			dMap22 := PROJECT(dMap22_temp, TRANSFORM(ClusterPair, SELF.id := LEFT.x; SELF.clusterid := LEFT.y; SELF.number := 0; SELF.value01 := LEFT.value; SELF.value02 := 0; SELF.value03 := 0;));		
      OUTPUT(dMap22, NAMED('dMap22'));			
			dMappedDistances22 := SORT(MappedDistances(d01,dCentroid2,fDist,dMap22), x, value);	
			OUTPUT(dMappedDistances22, NAMED('dMappedDistances22'));
			ub2_changed_final := ML.Cluster.Closest(dMappedDistances22);
      ...
      OUTPUT(ub2, NAMED('ub2'));
			...
			lbs2_changed_temp := JOIN(dMappedDistances21, ub2_changed, LEFT.x = RIGHT.x, TRANSFORM(LEFT), LEFT ONLY);
			lbs2_changed_temp1 := JOIN(lbs2_changed_temp, Gt, LEFT.y = RIGHT.x, TRANSFORM(Mat.Types.Element, SELF.x := LEFT.x, SELF.y := RIGHT.y, SELF.value := LEFT.value ));
			OUTPUT(lbs2_changed_temp, NAMED('lbs2_changed_temp'));
			lbs2_changed:= DEDUP(SORT(lbs2_changed_temp1,x,y,value),x,y);
      ...
      dJoined2:=JOIN(dRolled2,dClusterCounts2,LEFT.id=RIGHT.y,TRANSFORM(Types.NumericField,SELF.value:=LEFT.value/RIGHT.c;SELF:=LEFT;),LOOKUP);
		  dPass2:=JOIN(dCentroid2,TABLE(dJoined2,{id},id,LOCAL),LEFT.id=RIGHT.id,TRANSFORM(LEFT),LEFT ONLY,LOOKUP);
			dCentroid3 := dPass2 + dJoined2;
      OUTPUT(dCentroid3, NAMED('dCentroid3')); 
			
//itr3
c3 := 3;			
			dAddedx2 := JOIN(dAddedx1, dCentroid3, LEFT.id = RIGHT.id AND LEFT.number=RIGHT.number,TRANSFORM(lIterations,SELF.values:=LEFT.values+[RIGHT.value];SELF:=LEFT;));
			//calculate the deltaC
			deltac2 := dDistanceDelta(c3,c3-1,dAddedx2);
			OUTPUT(deltac2,NAMED('deltac2'));			
			bConverged2 := IF(MAX(deltac2,value)<=nConverge,TRUE,FALSE);

			deltacg3 :=JOIN(deltac2, Gt, LEFT.id = RIGHT.x, TRANSFORM(Mat.Types.Element,SELF.value := LEFT.value; SELF := RIGHT;));
			OUTPUT(deltacg3, NAMED('delatcg3'));
			deltacGt3 := SORT(deltacg3, y,-value);
			output(deltacGt3,NAMED('deltacGt3'));
			deltaG3 := DEDUP(deltacGt3,y);
			OUTPUT(deltaG3,NAMED('deltaG3')); 

			//update ub0 and lbs0
			ub3_temp := JOIN(ub2, deltac2, LEFT.y = RIGHT.id, TRANSFORM(Mat.Types.Element, SElF.value := LEFT.value + RIGHT.value; SELF := LEFT;));
			OUTPUT(ub3_temp, NAMED('ub3_temp'));
			lbs3_temp := JOIN(lbs2, deltaG3, LEFT.y = RIGHT.y , TRANSFORM(Mat.Types.Element, SELF.value := ABS(LEFT.value - RIGHT.value); SELF := LEFT));
			OUTPUT(lbs3_temp, NAMED('lbs3_temp'));
		
			// groupfilter1
			//calculate the 'b(x) for all x
			dMap31 := PROJECT(ub2, TRANSFORM(ClusterPair, SELF.id := LEFT.x; SELF.clusterid := LEFT.y; SELF.number := 0; SELF.value01 := LEFT.value; SELF.value02 := 0; SELF.value03 := 0;));		
			dMappedDistances31 := SORT(MappedDistances(d01,dCentroid3,fDist,dMap31), x, value);	
			OUTPUT(dMappedDistances31, NAMED('dMappedDistances31'));
			ub3_changed_temp := ML.Cluster.Closest(dMappedDistances31);
			OUTPUT(ub3_changed_temp);
      groupFilter3 := JOIN(lbs3_temp, ub3_temp,LEFT.x = RIGHT.x AND (LEFT.value < RIGHT.value), TRANSFORM(Mat.Types.Element, SELF := LEFT));	
      OUTPUT(groupFilter3, NAMED('groupFilter3'));			
			
			dMap32_temp := JOIN(groupFilter3, Gt, LEFT.y = RIGHT.y, TRANSFORM(Mat.Types.Element, SELF.x := LEFT.x, SELF.y := RIGHT.x, SELF.value := LEFT.value ));
			dMap32 := PROJECT(dMap32_temp, TRANSFORM(ClusterPair, SELF.id := LEFT.x; SELF.clusterid := LEFT.y; SELF.number := 0; SELF.value01 := LEFT.value; SELF.value02 := 0; SELF.value03 := 0;));		
      OUTPUT(dMap32, NAMED('dMap32'));			
			dMappedDistances32 := SORT(MappedDistances(d01,dCentroid3,fDist,dMap32), x, value);	
			OUTPUT(dMappedDistances32, NAMED('dMappedDistances32'));
			ub3_changed_final := ML.Cluster.Closest(dMappedDistances32);
			OUTPUT(ub3_changed_final, NAMED('ub3_changed_final'));
			ub3_changed := JOIN(ub3_changed_temp, ub3_changed_final, LEFT.x = RIGHT.x AND LEFT.value > RIGHT.value, TRANSFORM(RIGHT));
      ...
      OUTPUT(ub3, NAMED('ub3'));
			...
			lbs3_changed_temp := JOIN(dMappedDistances31, ub3_changed, LEFT.x = RIGHT.x, TRANSFORM(LEFT), LEFT ONLY);
			lbs3_changed_temp1 := JOIN(lbs3_changed_temp, Gt, LEFT.y = RIGHT.x, TRANSFORM(Mat.Types.Element, SELF.x := LEFT.x, SELF.y := RIGHT.y, SELF.value := LEFT.value ));
			OUTPUT(lbs3_changed_temp, NAMED('lbs3_changed_temp'));
			lbs3_changed:= DEDUP(SORT(lbs3_changed_temp1,x,y,value),x,y);
			lbs3_unchanged:= JOIN(lbs3_temp, lbs3_changed, LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y, LEFT ONLY);
      lbs3 := lbs3_unchanged + lbs3_changed;			
