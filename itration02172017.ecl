...
c3 := 3;			
			...
			//calculate the deltaC
			deltac2 := dDistanceDelta(c3,c3-1,dAddedx1);
			OUTPUT(deltac2,NAMED('deltac2'));			
			bConverged2 := IF(MAX(deltac2,value)<=nConverge,TRUE,FALSE);

			deltacg3 :=JOIN(deltac1, Gt, LEFT.id = RIGHT.x, TRANSFORM(Mat.Types.Element,SELF.value := LEFT.value; SELF := RIGHT;));
			...
			dMappedDistances31 := SORT(MappedDistances(d01,dCentroid2,fDist,dMap31), x, value);	
      ...
      OUTPUT(dMap32, NAMED('dMap32'));			
			dMappedDistances32 := SORT(MappedDistances(d01,dCentroid2,fDist,dMap32), x, value);	
