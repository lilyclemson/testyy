...
/*		  
			lLocalFilter:=RECORD 
			TYPEOF(Types.NumericField.id) x; //x.id
			TYPEOF(Types.NumericField.id) y; //g.id
			TYPEOF(Types.NumericField.id) z; //c.id
			TYPEOF(Types.NumericField.value) value; //
			TYPEOF(Types.NumericField.value) flag;  //if the data point changed its lb: 1 means it did not pass the local filter, if -1 means it passed the local filter
			END;
			
			groupGt := JOIN(pGroupFilter1, DeltacGt, LEFT.y = RIGHT.y, TRANSFORM(lLocalFilter, SELF.x := LEFT.x, SELF.y := LEFT.y, SELF.z := RIGHT.x, SELF.value := RIGHT.value, SELF.flag := 0), LEFT OUTER);			
      dMap1 := PROJECT(groupGt, TRANSFORM(ClusterPair, SELF.id := LEFT.x; SELF.clusterid := LEFT.z;SELF.number := 0; SELF.value01 := LEFT.value; SELF.value02 := 0; SELF.value03 := 0;));		
			dMappedDistances1 := MappedDistances(d01,dCentroid1,fDist,dMap1);	
			OUTPUT(dMappedDistances1, NAMED('dMappedDistances'));
			
			ub1_changedSet_temp := DEDUP(SORT(dMappedDistances1, x, value),x);
			ub1_changedSet := JOIN(ub1_changedSet_temp, ub0, LEFT.x = RIGHT.x AND LEFT.y !=RIGHT.y, TRANSFORM(LEFT));
			ub1_unChangedSet := JOIN(ub1_temp, ub1_changedSet, LEFT.x = RIGHT.x, TRANSFORM(LEFT), LEFT ONLY);
//      ub1:= JOIN(ub1_temp, ub1_changedSet, LEFT.x = RIGHT.x, TRANSFORM(Mat.Types.Element, SELF.x := LEFT.x; SELF.y := (LEFT.y=RIGHT.y?LEFT.y:RIGHT.y); SELF.value := (LEFT.y=RIGHT.y?LEFT.value:RIGHT.value);), LEFT ONLY);			
      ub1 := ub1_changedSet + ub1_unChangedSet;
			
			lb1_changedSet_tempa:= JOIN(dMappedDistances1, ub1_changedSet, LEFT.x = RIGHT.x, TRANSFORM(LEFT), LEFT ONLY);
      lb1_changedSet_tempb := DEDUP(SORT(lb1_changedSet_tempa, x, value),x);
			lb1_changedSet := JOIN(lb1_changedSet_tempb, Gt, LEFT.y = RIGHT.x, TRANSFORM(Mat.Types.Element, SELF.y := RIGHT.y, SELF:= LEFT;));
			lb1_unChangedSet := JOIN(lbs1_temp, lb1_changedSet, LEFT.x = RIGHT.x AND LEFT.y != RIGHT.y, TRANSFORM(LEFT), LEFT ONLY);
			lbs1 := lb1_changedSet + lb1_unChangedSet;
//			lbs1= JOIN(lbs1_temp, lb1_changedSet, LEFT.x = RIGHT.x, TRANSFORM(Mat.Types.Element, SELF.x := LEFT.x; SELF.y := (LEFT.y == RIGHT.y?LEFT.y:RIGHT.y); SELF.value := (LEFT.y == RIGHT.y?LEFT.value:RIGHT.value);), LEFT ONLY);
			
			//Calculate Vin: the number of data points who move into a new cluster					
			dClusterCountsVin:=TABLE(ub1_changedSet, {y; INTEGER c := COUNT(GROUP)},y);
			// Join closest to the document set and replace the id with the centriod id
			dClusteredVin := JOIN(d01, ub1_changedSet, LEFT.id = RIGHT.x,TRANSFORM(Types.NumericField,SELF.id:=RIGHT.y;SELF:=LEFT;));
			// Now roll up on centroid ID, summing up the values for each axis
			...
			VoutSet := JOIN(ub1_temp, ub1_changedSet, LEFT.x = RIGHT.x, TRANSFORM(LEFT));
			...
			dClusterCountsV :=JOIN(V0,dClusterCountsVin, LEFT.x = RIGHT.y, TRANSFORM(RECORDOF(LEFT), SELF.y := LEFT.y +RIGHT.c; SELF.value := LEFT.value + RIGHT.c; SELF := LEFT;), LEFT OUTER );
			...
			//Join to cluster counts to calculate the new average on each axis
			dClustered :=JOIN(dCentroid1, V0, LEFT.id = RIGHT.x, TRANSFORM(RECORDOF(LEFT), SELF.value := LEFT.value * RIGHT.y; SELF := LEFT), LEFT OUTER);
			//Add Vin 
		  ...
		  dCentroidOut:=JOIN(dJoined,dClusterCountsUpdate,LEFT.id=RIGHT.x,TRANSFORM(RECORDOF(LEFT),SELF.value:=(LEFT.value/RIGHT.y);SELF:=LEFT;), LEFT OUTER);
					...
					dClusterCountout := JOIN(iV, dClusterCountsUpdate, LEFT.x = RIGHT.x ,TRANSFORM(lInput,SELF.id := 4;SELF.values:=LEFT.values+[RIGHT.value];SELF.y := RIGHT.y; SELF:=LEFT;));
					//Integrate each dataset into one dataset as the output dataset
					dOutput := dCentroidsOut+ dUbOut + dLbsOut +dClusterCountout ;	
					OUTPUT(dOutput);
/*		
			//**************correct LocalFitler
			LocalFilterTemp1 := JOIN(groupFilter1,lbs0, LEFT.y = RIGHT.y, TRANSFORM(Mat.Types.Element,SELF.x := LEFT.x, SELF.y := LEFT.y, SELF.value := RIGHT.value),LEFT OUTER);
      OUTPUT(LocalFilterTemp1, NAMED('LocalFilterTemp1'));
			//the lbs of the data poits that filted by group fitler 1:m
			LocalFilterTemp2 := JOIN(LocalFilterTemp1, DeltacGt, LEFT.y = RIGHT.y, TRANSFORM(Mat.Types.Element, SELF.x := LEFT.x, SELF.y := RIGHT.x, SELF.value := LEFT.value - RIGHT.value), LEFT OUTER);			
      OUTPUT(LocalFilterTemp2, NAMED('LocalFilterTemp2'));		
			//get the delta of centroids of each group for each points and calculate the lb(x) - delta(c) )
			lLocalFilter:=RECORD 
			TYPEOF(Types.NumericField.id) x; //x.id
			TYPEOF(Types.NumericField.id) y; //g.id
			TYPEOF(Types.NumericField.id) z; //c.id
			...
			lLocalFilter transFormat(Mat.Types.Element input , UNSIGNED c) := TRANSFORM
			SELF.id := c;
			SELF.values := [input.value];
			SELF := input;
			END;
			
			LocalFilterTemp3 := JOIN(LocalFilterTemp2, lb0, LEFT.x = RIGHT.x, TRANSFORM(lLocalFilter, SELF.x := LEFT.x, SELF.y := LEFT.y, SELF.z = RIGHT.x, SELF.value := LEFT.value - RIGHT.value, SELF.flag := (LEFT.value - RIGHT.value)?1:-1)), LEFT OUTER );
      OUTPUT(LocalFilterTemp3, NAMED('LocalFilterTemp3'));	
			
			LocalFilterTemp3 := JOIN(LocalFilterTemp2, DeltacGt, LEFT.y = RIGHT.y, TRANSFORM(lLocalFilter, SELF.x := LEFT.x, SELF.y := LEFT.x, SELF.z = SELF.value :=LEFT.value - RIGHT.value), LEFT OUTER );
      OUTPUT(LocalFilterTemp3, NAMED('LocalFilterTemp3'));	
			
			//****correct localfilter 
			//localfilter1: get the centroids pass localfilter 
			LocalFilter1 := JOIN(LocalFilterTemp3,lb0, LEFT.x = RIGHT.x AND LEFT.value-RIGHT.value < 0, TRANSFORM(Mat.Types.Element, SELF.value := LEFT.value-RIGHT.value , SELF := LEFT));
			OUTPUT(LocalFilter1, NAMED('LocalFilter1'));
			//localfilter2: get update lbs of all centroids pass the group filter
			LocalFilter2temp := JOIN(LocalFilterTemp3,lb0, LEFT.x = RIGHT.x, TRANSFORM(Mat.Types.Element, SELF.value := LEFT.value-RIGHT.value>0? RIGHT.value:LEFT.value, SELF := LEFT));
			OUTPUT(LocalFilter2temp, NAMED('LocalFilter2temp'));
			LocalFilter2 := (SORT(LocalFilter1temp, x, y, value), x);
			OUTPUT(LocalFilter2, NAMED('LocalFilter2'));
			dMap1 := PROJECT(LocalFilter1, TRANSFORM(ClusterPair, SELF.id := LEFT.x; SELF.clusterid := LEFT.y;SELF.number := 0; SELF.value01 := LEFT.value; SELF.value02 := 0; SELF.value03 := 0;));		
			dMappedDistances1 := MappedDistances(d01,dCentroid1,fDist,dMap1);	
			OUTPUT(dMappedDistances1, NAMED('dMappedDistances'));
			
			//update ub and lb of the data points passed the local filter
			ub1_filtered := DEDUP(SORT(dMappedDistances1, x, value), x) ;
			lb1_filteredTemp := JOIN(SORT(dMappedDistances1, x, value), ub1_filtered, LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y, LEFT ONLY);
			lb1_filtered := DEDUP(lb1_filteredTemp, x);
			OUTPUT(ub1_filtered, NAMED('ub1_filtered'));
			OUTPUT(lb1_filteredTemp, NAMED('lb1_filteeredTemp'));
			OUTPUT(lb1_filtered, NAMED('lb1_filtered'));
