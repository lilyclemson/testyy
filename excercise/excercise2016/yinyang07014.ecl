...
dRolledVin;
// dJoinedVin:=JOIN(dRolledVin,dClusterCountsVin,LEFT.id=RIGHT.cvin,TRANSFORM(Types.NumericField,SELF.value:=LEFT.value/RIGHT.c;SELF:=LEFT;));
...
dRolledVout;
// dJoinedVout:=JOIN(dRolledVout,dClusterCountsVout,LEFT.id=RIGHT.cvout,TRANSFORM(Types.NumericField,SELF.value:=LEFT.value/RIGHT.c;SELF:=LEFT;));
...
dAdded2:=JOIN(minusC,Vcount4,LEFT.id=RIGHT.id,TRANSFORM(RECORDOF(LEFT),SELF.value:=(LEFT.value/RIGHT.c);SELF:=LEFT;), LEFT OUTER);
dAdded2;

totaldAdded := JOIN(dAdded1,dAdded2,LEFT.id=RIGHT.id AND LEFT.number=RIGHT.number,TRANSFORM(lIterations,SELF.values:=LEFT.values+[RIGHT.value];SELF:=RIGHT;), LEFT OUTER);
totaldAdded;
