...
//get lbs0_ini
lbs0_iniTemp := JOIN(dClosest0,dDistances0, LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y,RIGHT ONLY);// result is already sorted based on x first and y second.
lbs0_iniTemp1 := SORT(lbs0_iniTemp,x, value);
...
lInput yyfIterate(DATASET(lInput) d,UNSIGNED c):=FUNCTION
			...
			lbs1_temp := PROJECT(lbs0, TRANSFORM(Mat.Types.Element, SELF.value := LEFT.value - deltaG1; SELF := LEFT));
			...
			// groupFilter1_Converge := IF(COUNT(groupFilter1)=0,  TRUE, FALSE);

			//localFilter
			changedSetTemp := JOIN(d01,groupFilter1, LEFT.id = RIGHT.x,TRANSFORM(LEFT));
			// changedSetTemp;
			...
			//new Ub
			ub1:= SORT(LocalFilter1 + unChangedUbRolled, x);
			...
			changelbsTemp11 := SORT(JOIN(localFilter1,dDistances1, LEFT.x = RIGHT.x AND LEFT.y = RIGHT.y, RIGHT ONLY), x, value);
			...
			dClusterCountsVin1:=TABLE(localFilter1, {y; INTEGER c := COUNT(GROUP)},y);
			// OUTPUT(dClusterCountsVin1,NAMED('dClusterCountsVin1'));
			dClusteredVin1 := JOIN(d01, localFilter1, LEFT.id = RIGHT.x,TRANSFORM(Types.NumericField,SELF.id:=RIGHT.y;SELF:=LEFT;));
			...
			//reduce SORT
			// RETURN IF( bConverged,  d, outputSet);
			RETURN MAP( MAX(deltac,value)<=nConverge => d, COUNT(groupFilter1)=0 => d,COUNT(localFilter1) =0 =>d , outputSet );

END;

yyResult :=LOOP(input0,n-1,yyfIterate(ROWS(LEFT),COUNTER));

OUTPUT(yyResult(id =1), NAMED('yyResult'));
