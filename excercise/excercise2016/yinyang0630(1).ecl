...
KmeansD01 := ML.Cluster.KMeans(d01,dCentroid,n,nConverge);
...
dClusteredVin:=SORT(DISTRIBUTE(JOIN(d01,Vin,LEFT.id=RIGHT.xid,TRANSFORM(Types.NumericField,SELF.id:=RIGHT.id;SELF:=LEFT;),HASH),id),RECORD,LOCAL);	
...
dClusteredVout:=SORT(DISTRIBUTE(JOIN(d01,Vout,LEFT.id=RIGHT.xid,TRANSFORM(Types.NumericField,SELF.id:=RIGHT.id;SELF:=LEFT;),HASH),id),RECORD,LOCAL);
...
Types.NumericField transVinVout(dRolledVin vin, dRolledVout vout) := TRANSFORM
SELF.value := vin.value - vout.value;
SELF := vin;
END;
dJoinedVinVout:=JOIN(dRolledVin,dRolledVout,LEFT.id = RIGHT.id AND LEFT.number=RIGHT.number,transVinVout(left,right),LOOKUP);
dJoinedVinVout;// ***********************************************************result18


Types.NumericField transCnew(dRolledV vold, dJoinedVinVout vnew) := TRANSFORM
SELF.value := vold.value + vnew.value;
SELF := vold;
END;


dJoinedVnew := JOIN(dRolledV,dJoinedVinVout,LEFT.id = RIGHT.id AND LEFT.number=RIGHT.number,transCnew(LEFT, RIGHT), LEFT ONLY);
dJoinedVnew;// ***********************************************************result19
//Join to cluster counts to calculate the new average on each axis for C'
dJoined:=JOIN(dJoinedVnew,V,LEFT.id=RIGHT.y,TRANSFORM(Types.NumericField,SELF.value:=LEFT.value/RIGHT.c;SELF:=LEFT;),LOOKUP);
dJoined;	// ***********************************************************result20					
...
END;
deltaCg :=JOIN(deltaC, Gt, LEFT.id = RIGHT.x,{id;y;value},y );
