...
lInput yyfIterate(DATASET(lInput) d,UNSIGNED c):=FUNCTION
			...
			unChangedUbD01CTemp := SORT(JOIN(unChangedUbD01, dCentroid1, LEFT.cid = RIGHT.id AND LEFT.number = RIGHT.number , TRANSFORM(Mat.Types.Element, SELF.value := POWER(LEFT.value-RIGHT.value, 2); SELF.y := RIGHT.id;SELF.x := LEFT.id;)),x);
			...
			// RETURN IF(bConverged, SORT(dAddedUb1,id), SORT(dAddedUb1,id));
			// RETURN dAddedLbs1;
END;

yyResult :=LOOP(input0,30,yyfIterate(ROWS(LEFT),COUNTER));
