...
Gt := TABLE(KmeansDt.Allegiances(), {x,y});//the assignment of each centroid to a group x: cid, y: gid
...
lJoinlb1 := RECORD
joinlb.x;
REAL value := MIN(GROUP, joinlb.value);
END;

joinlb1 := TABLE(joinlb, lJoinlb1, x);
joinlb1;

joinlb2 := JOIN(joinlb, joinlb1, LEFT.x = RIGHT.x AND LEFT.value = RIGHT.value, TRANSFORM(lBound, SELF.xid := LEFT.x; SELF.cid := LEFT.y; SELF.lb := LEFT.value; SELF.gid := 0;),RIGHT OUTER);
joinlb2;


lBound transLbound(joinlb2 j2, Gt gt) := TRANSFORM
SELF.gid := gt.y;
SELF := j2;
END;
//join result 9 and result 5 to get the dataset for lb
jDistlb2G := JOIN(joinlb2, Gt, LEFT.cid = RIGHT.x, transLbound(LEFT,RIGHT));

jDistG := SORT(jDistlb2G, xid);//***************result 10**************all the lbs without group by Gt
jDistG;
...
jDistGSort := SORT(jDistG,lb);//***************************group by xid and gid and sort by lb 
lbs := DEDUP(jDistGSort,gid);// get lb of each group of each id
