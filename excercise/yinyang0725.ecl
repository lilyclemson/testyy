...
//update ub0 and lbs0
ub1_temp := JOIN(ub0, deltaC1Gt, LEFT.y = RIGHT.x, TRANSFORM(Mat.Types.Element, SElF.value := LEFT.value + RIGHT.value; SELF := LEFT;));
...
unChangedUb := JOIN(unChangedUbTemp1, ub0, LEFT.id = RIGHT.x, TRANSFORM(Mat.Types.Element, SELF.x := RIGHT.x; SELF.y := RIGHT.y; SELF.value := LEFT.value; ));
...
//update ub and lbs
ub2_temp := JOIN(ub1, deltaC2Gt, LEFT.y = RIGHT.x, TRANSFORM(Mat.Types.Element, SElF.value := LEFT.value + RIGHT.value; SELF := LEFT;));
OUTPUT(ub2_temp, NAMED('ub2_temp'));
lbs2_temp := JOIN(lbs1, deltaG2, LEFT.y = RIGHT.y , TRANSFORM(Mat.Types.Element, SELF.value := LEFT.value - RIGHT.value; SELF := LEFT;));
OUTPUT(lbs2_temp, NAMED('lbs2_temp'));

//Groupfilter
// groupFilterTempIt2 := JOIN(ub2_temp, lbs2_temp,LEFT.x = RIGHT.x, TRANSFORM(Mat.Types.Element, SELF.value := RIGHT.value - LEFT.value; SELF := LEFT));
// OUTPUT(groupFilterTempIt2,NAMED('groupFilterTempIt2'));
// groupFilterTempIt21:= groupFilterTempIt2(value <0);
// lbs2Temp := JOIN(lbs2_temp, groupFilterTempIt21, LEFT.x = RIGHT.x, TRANSFORM(LEFT));
// groupFilter2 := JOIN(ub1_temp, lbs2Temp, LEFT.x = RIGHT.x, TRANSFORM(Mat.Types.Element, SELF.value := RIGHT.value - LEFT.value; SELF := LEFT;));

//groupfilter2 test for comparison
groupFilterTempIt2 := JOIN(ub1_temp, lbs2_temp,LEFT.x = RIGHT.x, TRANSFORM(Mat.Types.Element, SELF.value := RIGHT.value - LEFT.value; SELF := LEFT));
// OUTPUT(groupFilterTempIt2,NAMED('groupFilterTempIt2'));
...
//the dataset that change their best centroid = result of local fitler		
localFilter2 := JOIN(dClosest2, ub1, LEFT.x = RIGHT.x AND LEFT.y !=RIGHT.y, TRANSFORM(LEFT));
...
// If the centroids have converged, simply pass the input dataset through
// to the next iteration.  Otherwise perform an iteration.
localFilter2_result :=IF(localFilter2_Converge,dAdded1,dAdded2);
