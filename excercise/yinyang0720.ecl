...
Vcount1 := TABLE(dClosest0, lVcount, y);
OUTPUT(Vcount1, NAMED('Vcount1')); 	

//calculate the Vin
VinSet := SORT(localFilter1,y);
dClusterCountsVin:=TABLE(VinSet, {y; INTEGER c := COUNT(GROUP)},y);
OUTPUT(dClusterCountsVin,NAMED('dClusterCountsVin'));

dClusteredVin := JOIN(d01, VinSet, LEFT.id = RIGHT.x,TRANSFORM(Types.NumericField,SELF.id:=RIGHT.y;SELF:=LEFT;));
OUTPUT(dClusteredVin,NAMED('dClusteredVin'));
// dClusteredVin;
...
VoutSet := JOIN(dClosest0, localFilter1, LEFT.x = RIGHT.x, TRANSFORM(LEFT));
// OUTPUT(VoutSet,NAMED('VoutSet'));
dClusterCountsVout:=TABLE(VoutSet, {y; INTEGER c := COUNT(GROUP)},y);
OUTPUT(dClusterCountsVout,NAMED('dClusterCountsVout'));
dClusteredVout := JOIN(d01,VoutSet, LEFT.id = RIGHT.x,TRANSFORM(Types.NumericField,SELF.id:=RIGHT.y;SELF:=LEFT;));
OUTPUT(dClusteredVout,NAMED('dClusteredVout'));
...
OUTPUT(dRolledVout,NAMED('dRolledVout'));

//calculate the new Vcount = old Vcount + |Vin| - |Vout| 
Vcount2Vin :=JOIN(Vcount1,dClusterCountsVin, LEFT.id = RIGHT.y, TRANSFORM(RECORDOF(LEFT), SELF.inNum := RIGHT.c;  SELF := LEFT;), LEFT OUTER );
OUTPUT(Vcount2Vin,NAMED('Vcount2Vin'));
Vcount2VinVout :=JOIN(Vcount2Vin,dClusterCountsVout, LEFT.id = RIGHT.y, TRANSFORM(RECORDOF(LEFT), SELF.outNum := RIGHT.c; SELF := LEFT;), LEFT OUTER);
OUTPUT(Vcount2VinVout,NAMED('Vcount2VinVout'));
