...
// lInput yyfIterate(DATASET(lInput) d,UNSIGNED c):=FUNCTION
			...
			Vin1Temp :=JOIN(Vin0,dClusterCountsVin1, LEFT.x = RIGHT.y, TRANSFORM(RECORDOF(LEFT), SELF.value := LEFT.value +RIGHT.c; SELF.y := LEFT.y + RIGHT.c; SELF := LEFT;), LEFT OUTER );
			OUTPUT(Vin1Temp,NAMED('Vin1'));
			Vout1:=JOIN(Vin1Temp,dClusterCountsVout1, LEFT.x = RIGHT.y, TRANSFORM(RECORDOF(LEFT), SELF.value := LEFT.value +RIGHT.c; SELF.y := LEFT.y - RIGHT.c;SELF := LEFT;), LEFT OUTER);
			OUTPUT(Vout1,NAMED('Vout'));
			
			Vin1 := JOIN(Vin1Temp, Vout1, LEFT.x = RIGHT.x, TRANSFORM(RECORDOF(LEFT), SELF.y := RIGHT.y; SELF := LEFT;), LEFT OUTER);


			//let the old |c*V| to multiply old Vcount
			CV1 :=JOIN(dCentroid1, Vin0, LEFT.id = RIGHT.x, TRANSFORM(RECORDOF(LEFT), SELF.value := LEFT.value * RIGHT.y; SELF := LEFT), LEFT OUTER);
			...
			//get the new C
			newC:=JOIN(CV1VinVout,Vin1,LEFT.id=RIGHT.x,TRANSFORM(RECORDOF(LEFT),SELF.value:=(LEFT.value/RIGHT.y);SELF:=LEFT;), LEFT OUTER);
			...
			dAddedVin1 := JOIN(iVin, Vin1, LEFT.x = RIGHT.x ,TRANSFORM(lInput,SELF.id := 4;SELF.values:=LEFT.values+[RIGHT.value];SELF.y := RIGHT.y; SELF:=LEFT;));
			OUTPUT(dAddedLbs1, NAMED('dAddedVin1'));
			dAddedVout1 := JOIN(iVout, Vout1, LEFT.x = RIGHT.x ,TRANSFORM(lInput,SELF.id := 5;SELF.values:=LEFT.values+[RIGHT.value];SELF.y := RIGHT.y; SELF:=LEFT;));
			OUTPUT(dAddedLbs1, NAMED('dAddedVout1'));
			...
			outputSet := dAddedx1+ dAddedUb1 + dAddedLbs1 +dAddedVin1 + dAddedVout1;
			SORT(outputSet,id);
